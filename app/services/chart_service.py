...
class ChartGenerator:
    ...
    def _is_explicit_chart_request(self, query: str) -> bool:
        q = query.lower()
        ...
        count = sum(1 for k in mapping if k in q)

        # Only treat as single-chart request if EXACTLY ONE chart type is mentioned
        return count == 1
    
    def _is_multi_chart_request(self, query: str) -> bool:
        q = query.lower()
        ...
        count = sum(1 for k in chart_keywords if k in q)

        return count > 1
    
    def _extract_requested_charts(self, query: str):
        q = query.lower()
    ...
    def _should_show_tables(self, query: str) -> bool:
        ...
        return self._query_matches(query, self._PIVOT_INTENT | self._SUMMARY_INTENT)

    def _is_dashboard_query(self, query: str) -> bool:
        return self._query_matches(query, self._BROAD_INTENT)

    def _augment_query(self, query: str) -> str:
        if self._is_explicit_chart_request(query):
            return query  # never augment — explicit queries must not be forced to multi-chart
    ...
    def _needs_table(self, query: str) -> bool:
        return self._query_matches(query, self._PIVOT_INTENT)
    ...
    def _process_sync(self, llm_schema: LLMResponseSchema, query: str) -> dict:
        ...
        # Step 3: Gate scorecards on intent
        scorecards = (
            self._scorecard_builder.build()
            if self._should_show_scorecards(query)
            else []
        )

        # Step 5: Limit schema for explicit chart requests
        if self._is_explicit_chart_request(query):
            # user wants exactly one chart
            llm_schema.charts = llm_schema.charts[:1]
            llm_schema.tables = []
            scorecards = []

        elif self._is_multi_chart_request(query):
            requested = self._extract_requested_charts(query)

            logger.info("Multi-chart request detected: %s", requested)

            # Keep only requested charts from LLM
            llm_schema.charts = [
                c for c in llm_schema.charts if c.type in requested
            ]

            existing = {c.type for c in llm_schema.charts}

            # Add missing ones
            for t in requested:
                if t not in existing:
                    fallback = self._fallback_chart_config(t)
                    if fallback:
                        llm_schema.charts.append(fallback)
                        logger.info("Added missing chart: %s", t)

        charts = self._build_charts(llm_schema)

        # Step 6: Post-process charts
        charts = self._post_process_charts(charts, self.data)
    ...
    def _post_process_charts(self, charts: list, df) -> list:
        """Step 6: Filter weak/duplicate charts from LLM output."""
        filtered = []
        for c in charts:
            # Remove weak scatter plots (too few unique values to be meaningful)
            if c.get("type") == "scatter":
                ...
                if x_col in df.columns and y_col in df.columns:
                    if df[x_col].nunique() < 5 or df[y_col].nunique() < 5:
                        logger.info("Dropping weak scatter: x=%r y=%r", x_col, y_col)
                        continue
            filtered.append(c)

        # Remove duplicates (same type + x + y)
        unique = []
        seen = set()
        for c in filtered:
            ...
            if key not in seen:
                ...
                logger.info("Dropping duplicate chart: %r", key)
    ...
    def _fallback_chart_config(self, chart_type: str):
        num_cols = self.data.select_dtypes(include="number").columns
        ...
        if chart_type == "pie" and len(cat_cols) and len(num_cols):
            return {
                "type": "pie",
                "x": cat_cols[0],
                "y": num_cols[0],
        ...
        if chart_type == "bar" and len(cat_cols) and len(num_cols):
            return {
                "type": "bar",
                "x": cat_cols[0],
                "y": num_cols[0],
